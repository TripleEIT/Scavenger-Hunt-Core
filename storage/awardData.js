import { getDefaultAward, hasDefaultAwardSuggestion } from '../defaults/awardSuggestions.js';
import { generateRandomString } from '../utils.js';
import { getConfigurationSettings, setConfigurationSettings } from './configurationData.js';
import { deleteDistributedData, getDistributedData, setDistributedData } from './distributedStorage.js';
import { getNarrowUserRecord, getUserWinCount } from './userData.js';

const awardPrefix = 'awardSettings';
export const replaceUndefinedValues = (formData, awardOptionState) => {
    const result = {};
...
export const getNarrowAward = (awardId, awardOptions) => {
    return {
        id: awardId,
...
export const getAwardOption = async (awardId, context) => {
    try {
...
export const deleteAwardOption = async (awardId, context) => {
    try {
...
export const saveAwardOption = async (awardId, awardOption, context) => {
    try {
...
export const getActiveProductAwards = async (product, context, configuration) => {
    if (!configuration) {
...
export const recordAwardedIncrementWon = async (standardEvent, awardWon, context, configuration, updatedUser) => {
    try {
